...
export default function CameraToggle() {
  const { showCameras, setShowCameras } = useNavigationStore();
  ...
  return (
    ...
    >
      <motion.button
        ...
        aria-label={showCameras ? 'Hide surveillance cameras' : 'Show surveillance cameras'}
        ...
        style={{
          background: showCameras
            ? 'rgba(6,182,212,0.18)'
            : 'rgba(15,23,42,0.82)',
          borderColor: showCameras
            ? 'rgba(6,182,212,0.45)'
            : 'rgba(255,255,255,0.1)',
          boxShadow: showCameras
            ? '0 0 0 1px rgba(6,182,212,0.3), 0 0 18px rgba(6,182,212,0.15), 0 4px 16px rgba(0,0,0,0.4)'
      ...
      >
        ...
        <motion.div
          animate={{ scale: showCameras ? 1 : 0.9 }}
          transition={{ type: 'spring', stiffness: 300, damping: 20 }}
        >
          {showCameras
            ? <Video size={20} style={{ color: '#06b6d4' }} />
            : <VideoOff size={20} style={{ color: '#9CA3AF' }} />
          }
        </motion.div>
      </motion.button>
